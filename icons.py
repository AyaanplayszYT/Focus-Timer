...
class IconPainter:
    ...
    @staticmethod
    def draw_search(painter: QPainter, rect: QRectF, color: QColor):
        ...
        # Draw circle (lens)
        lens_cx = cx - size * 0.15
        lens_cy = cy - size * 0.15
        lens_radius = size * 0.55
        painter.drawEllipse(QRectF(lens_cx - lens_radius, lens_cy - lens_radius, 
                                   lens_radius * 2, lens_radius * 2))
        
        # Draw handle
        import math
        handle_start_x = lens_cx + lens_radius * math.cos(math.pi / 4)
        handle_start_y = lens_cy + lens_radius * math.sin(math.pi / 4)
        handle_end_x = cx + size * 0.7
        handle_end_y = cy + size * 0.7
